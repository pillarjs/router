...
Layer.prototype._handle_any = function _handle_any(/* ...args, next */) {
  ...
  var result
  function hasNextBeenCalled() { return next._currentLayer === self }

  try {
    result = handle.apply((function () { return this })(), arguments)
  } catch (err) {
    return next(err)
  }
  ...
  if (result != null && typeof result.then === 'function') {
    result.then(
      null,
      function onRejected(error) { if (!hasNextBeenCalled()) { next(error || new Error('Promise rejected with falsey value')) } }
    )
  }
}
