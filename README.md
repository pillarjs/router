...
Use the given [middleware function](#middleware) for all http methods on the given `path`,
...
format is with three parameters - "request", "response" and "next".

- `request`  - This is a [HTTP incoming message](https://nodejs.org/api/http.html#http_http_incomingmessage) instance
- `response` - This is a [HTTP server response](https://nodejs.org/api/http.html#http_class_http_serverresponse) instance
...
subsequent arguments remain, becoming - "error", "request", "response", "next".
These functions are "error handling middleware", and can be used for handling
errors that occurred in previous handlers (E.g. from calling `next(error)`).
